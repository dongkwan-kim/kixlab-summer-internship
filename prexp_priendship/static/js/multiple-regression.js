...
function multiple_regression(x_arr_list, y_arr){
	/*
	x_arr_list = [[..], [..], ..]
	y_arr = [...]
	*/
	var regression = new Regression({
		numX: x_arr_list.length,
		numY: 1
	});

	for(var idx = 0; idx < x_arr_list[0].length; idx++){
		var x_arr = [];
		for(var jdx = 0; jdx < x_arr_list.length; jdx++){
			x_arr.push(x_arr_list[jdx][idx]);
		}
		regression.push({ x: x_arr, y: [y_arr[idx]]})
	}	
